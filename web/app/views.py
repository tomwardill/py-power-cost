...
def data_month(request):
    ...
    while previous_month < now:
        ...
        data = Reading.objects.filter(time__range = (previous_month, previous_plus_day)).aggregate(temperature_average = Avg('temperature'), day_average = Avg('ch1_wattage'), first_time = Min('time'))
        averaged_data.append(data)
    ...
    temp_data  = [[time.mktime(k['first_time'].timetuple()) * 1000, float(k['temperature_average'])] for k in averaged_data if k['first_time']]
    json_data = json.dumps([graph_data, temp_data])
